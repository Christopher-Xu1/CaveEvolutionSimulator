...
def run_simulation(
    ...
    preset_name,
    num_patches=1,
    fitness_threshold=0.5,  # This parameter can be deprecated if not used elsewhere
    egg_count=50,
    carrying_capacity=1000
):
    # Corrected Mutation Rate Assignment
    ...
    for generation in range(1, num_generations + 1):
        ...
        if total_fitness == 0:
            ...
            # Step 2: Assign survival probabilities
            for organism in population:
                organism.survival_probability = organism.fitness / total_fitness
            
            # Step 3: Determine survivors
            surviving_population = [org for org in population if random.random() < org.survival_probability]
        
        # Optional: Print debug information
        # print(f"Generation {generation}: Total Fitness = {total_fitness:.4f}")
        # print(f"Surviving Population Size = {len(surviving_population)}")
        ...
        # Calculate potential offspring counts based on surviving population
        for organism in surviving_population:
            ...
            reproductive_capability = max(1, (organism.fitness * 2) * ((egg_count) * (1 + food_availability) ** 3 / egg_count))
            # Introduce slight randomness to offspring count
            count = int(random.uniform(reproductive_capability - 1, reproductive_capability + 1))
            count = max(count, 1)  # Ensure at least one offspring
        ...
        for organism, num_offspring in zip(surviving_population, offspring_counts):
            for _ in range(num_offspring):
                parent = random.choice(surviving_population)
...
if __name__ == "__main__":
    ...
    run_simulation(
        ...
        num_patches=1,
        fitness_threshold=0.5,  # This parameter is now deprecated
        egg_count=50,
        carrying_capacity=1000
    )
