...
# Ensure correct initial population size
    ...
    for generation in range(1, num_generations + 1):
        ...
        egg_egg_survival_rate = 1/(egg_count)**(random.gauss(1, 1.5))
        ...
        for organism in viable_population:
            num_offspring = min( carrying_capacity-len(offspring_population),
                                max(1, int((organism.fitness * egg_count * egg_egg_survival_rate) * random.uniform(0.8, 1.2))))
        ...
        print(f"- Survival Rate: {egg_survival_rate:.4f}")
