...
class Organism:
    def __init__(self, genetics=None):
        if genetics is not None:
            ...
            self.genetics = {
                'pigmentation': random.gauss(0.5, 0.181),  # Regressive trait
                'eye_size': max(0,.1random.gauss(0.5, 0.181),      # Regressive trait
                'metabolic_rate': random.uniform(0.3, 0.7),  # Adaptive trait
                'lateral_line': random.gauss(0.5, 0.181),  # Adaptive trait
                'olfactory_bulb': random.gauss(0.5, 0.181) # Adaptive trait
            }
        self.fitness = 0
