...
def run_simulation(num_decades, initial_population_size, preset_name, num_patches=1, fitness_threshold=0.5,
    egg_count=50,
...
# Ensure correct initial population size
    ...
    for generation in range(1, num_generations + 1):
        ...
        survival_rate = max(0.1, 1 - (len(population) / carrying_capacity)**2)
        ...
        for organism in viable_population:
            num_offspring = min( carrying_capacity-len(offspring_population),
                                max(1, int((organism.fitness * egg_count * survival_rate) * random.uniform(0.8, 1.2))))
