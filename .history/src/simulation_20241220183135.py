...
def run_simulation(
    ...
    for generation in range(1, num_generations + 1):
        ...
        if  total_potential_offspring>carrying_capacity:
            total_potential_offspring=carrying_capacity
        ...
        resource_usage = (sum( for patch in environment.patches) * np.mean(trait_averages["metabolic_rate"])) / carrying_capacity
...
def plot_results_separate(population_sizes, average_fitness, trait_averages, food_availabitlity_over_time):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(generations, food_availabitlity_over_time, label="Resource Usage", color="purple", linewidth=2)
    plt.xlabel("Generation")
