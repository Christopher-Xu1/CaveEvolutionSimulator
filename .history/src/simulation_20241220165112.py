...
def run_simulation(
    ...
    # Example of initializing patches with unique IDs
    environment = Environment(num_patches=num_patches, preset=Environment.cave_presets(preset_name))
    for idx, patch in enumerate(environment.patches):
        patch['id'] = idx  # Assign a unique integer ID
...
# Initialize a dictionary to count organisms per patch using patch IDs
        ...
        # Define food consumption per organism
        food_consumption_per_org = 0.1  # Adjust based on simulation needs

        # Reduce food availability in each patch
        for patch in environment.patches:
            patch_id = patch['id']
            count = patch_population[patch_id]
            food_consumed = count * food_consumption_per_org
            patch['food_availability'] = max(patch['food_availability'] - food_consumed, 0)  # Clamp to 0
            # Optional: Print debug information
            # print(f"Patch {patch_id}: Food consumed = {food_consumed}, Remaining food = {patch['food_availability']}")
