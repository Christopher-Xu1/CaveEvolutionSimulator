...
# Ensure correct initial population size
    ...
    for generation in range(1, num_generations + 1):
        ...
        egg_survival_rate = 1/(egg_count)**(random.uniform(1,1+organism.environment_patch.get(food_availability)))
        ...
        for organism in viable_population:
            num_offspring = min( carrying_capacity-len(offspring_population),
                                max(1, int((organism.fitness * egg_count * egg_survival_rate) * random.gauss(0.5+organism.fitness, 0.2))))
