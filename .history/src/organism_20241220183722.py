...
class Organism:
    def __init__(self, genetics=None):
        if genetics is not None:
            ...
            self.genetics = {
                'pigmentation': max(0,random.gauss(0.6, 0.181)),  # Regressive trait
                'eye_size': max(0, random.gauss(0.6, 0.181)),      # Regressive trait
                'lateral_line': max(0,random.gauss(0.4, 0.181)),  # Adaptive trait
                'olfactory_bulb': max(0,random.gauss(0.4, 0.181)) # Adaptive trait
            }
    ...
    def calculate_fitness(self, environment_patch):
        ...
        W_r = (
            ...
            0.2 * (1 - self.genetics['eye_size'])
        )

        # Contribution from adaptive traits (sensory enhancements)
        W_a = (
            0.2 * (1 - abs(self.genetics['lateral_line'] - optimal_traits.get('lateral_line'))) + 
            0.2 * (1 - abs(self.genetics['olfactory_bulb'] - optimal_traits.get('olfactory_bulb')))
        )
