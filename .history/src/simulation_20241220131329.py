...
def run_simulation(
    ...
    for generation in range(1, num_generations + 1):
        ...
        if total_potential_offspring > carrying_capacity:
            offspring_counts = carrying_capacity
        ...
        # Collect data
      def collect_generation_metrics(population, generation, trait_averages):
    """Centralized metrics collection with error handling."""
    if not population:
        return None
    ...
    metrics = {
        ...
        'trait_averages': {
            trait: np.mean([org.genetics[trait] for org in population])
            for trait in trait_averages
        }
    }

    logging.info(f"Generation {generation}: {metrics}")
    return metrics

# Usage
generation_metrics = collect_generation_metrics(population, generation, trait_averages)
if generation_metrics:
    population_sizes.append(generation_metrics['population_size'])
    for trait, value in trait_averages.items():
        value.append(generation_metrics['trait_averages'][trait])
rait in trait_averages)}")
