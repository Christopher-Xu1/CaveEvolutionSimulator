...
def run_simulation(
    ...
    for idx, patch in enumerate(environment.patches):
        patch['id'] = idx
        patch['food_availability'] = 100.0  # Initialize food availability
    ...
    for generation in range(1, num_generations + 1):
        ...
        environment.replenish_food()  # Assuming this method exists and handles food replenishment
        ...
        # Calculate and store average traits
        for trait, values in trait_averages.items():
            if population:
                avg_trait = np.mean([org.genetics[trait] for org in population])
            else:
                avg_trait = 0
        ...
        for patch in environment.patches:
            ...
            # Ensure generation index is within bounds
            if generation - 1 < len(trait_averages["metabolic_rate"]):
                metabolic_rate = trait_averages["metabolic_rate"][generation - 1]
            else:
                metabolic_rate = 1.0  # Default value if out of range
            # Calculate food consumed
            food_consumed = (count * patch['food_availability'] * metabolic_rate) / carrying_capacity
        ...
        resource_usage = (sum(patch['food_availability'] for patch in environment.patches) * 
                          np.mean(trait_averages["metabolic_rate"])) / carrying_capacity
